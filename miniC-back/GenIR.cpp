...
Var *GenIR::genTwoOp(Var *lval, _TokenType opt, Var *rval)
{
    ...
    {
        if (lval->isVoid())
            SEMERROR(EXPR_IS_VOID, lval->name);
        else if (rval->isVoid())
    ...
    {
        if (!lval->isBase())
            SEMERROR(EXPR_NOT_BASE, lval->name);
        else if (!rval->isBase())
...
Var *GenIR::genAssign(Var *val) /**< 好像没什么必要 */
{
    ...
    symtab->addVar(temp);
    if (val->isPtr)
        symtab->addInst(new InterInst(OP_GET, temp, val->ptr));
    else
        symtab->addInst(new InterInst(OP_AS, temp, val));
    return temp;
}
...
void GenIR::genContinue()
{
    ...
    if (head)
        ...
        SEMERROR(CONTINUE_ERR, "continue");
}
