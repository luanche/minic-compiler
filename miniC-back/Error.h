...
public:
    static void LexError(int code)
    {
        ...
        cout<<" <"<<lex->getLineNum()<<"行,"<<lex->getColNum()<<"列> 词法错误:"<<lexErrorTable[code]<<".\n";
        highlight("",NORMAL);
    }
    static void SynError(int code,Token* t)
    {
        ...
        errorNum++;
        highlight("",ERRORCOLOR);
        if(code%2==0)  {cout<<" <"<<lex->getLineNum()<<"行> 语法错误: 在 ";
                        cout<< t->toString()<<" 之前丢失"<<synErrorTable[code/2]<<endl;}

        else cout<<" <"<<lex->getLineNum()<<"行> 语法错误: 在 "<< t->toString()<<" 处没有正确匹配"<<synErrorTable[code/2]<<endl;
        highlight("",NORMAL);
    }
    static void SemError(int code,string name)
    {
        ...
        errorNum++;
        highlight("",ERRORCOLOR);
        cout<<" <"<<lex->getLineNum()<<"行> 语义错误: "<< name<<" "<<SemErrorTable[code]<<endl;
        highlight("",NORMAL);
    }
    static void SemWarn(int code,string name)
    {
        ...
        cout<<" <"<<lex->getLineNum()<<"行> 语义警告: "<< name<<" "<<SemWarnTable[code]<<endl;
        highlight("",NORMAL);
    }
};
