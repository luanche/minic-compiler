...
void SymTab::addVar(Var *var)
{
    ...
    else
    {
        ...
        for (; i < _list.size(); i++)
        {
            if (_list[i]->scopepath.size() == 0 || var->scopepath.size() == 0)
                break;
            if (_list[i]->scopepath.back() == var->scopepath.back())
                break;
        }
    ...
    if (ir)
    {
        ...
        if (curFun && flag)
            curFun->locate(var); /**< 计算局部变量栈帧偏移 scopeEsp curEsp offset */
        //symtab->ShowIR();
    }
}
...
void SymTab::genAsm()
{
    ...
    {
        if (IOflag[i] == 1)
            fprintf(file, "\t.def	_putchar;	.scl	2;	.type	32;	.endef\n");
        else if (IOflag[i] == 2)
            fprintf(file, "\t.def	_puts;	.scl	2;	.type	32;	.endef\n");
        else if (IOflag[i] == 3)
            fprintf(file, "\t.def	_printf;	.scl	2;	.type	32;	.endef\n");
        else if (IOflag[i] == 4)
            fprintf(file, "\t.def	_scanf;	.scl	2;	.type	32;	.endef\n");
    }*/
}
...
void SymTab::ShowIR(bool outOpt)
{
    ...
    for (funIt = funTab.begin(); funIt != funEnd; ++funIt)
    {
        ...
        {
            if (intercode_copy[j]->label != "")
                cout << intercode_copy[j]->label << ":" << endl;
            else
            {
                cout << OperatorArray[int(intercode_copy[j]->op)] << " "; //cout<<(int)(intercode_copy[j]->op)<<" ";
                if (intercode_copy[j]->target != NULL)
                {
                    if (intercode_copy[j]->target->label != "")
                        cout << intercode_copy[j]->target->label << " ";
                }
                if (intercode_copy[j]->result != NULL)
                {
                    if (intercode_copy[j]->result->name[0] == '<')
                        cout << intercode_copy[j]->result->intVal << " ";
                    else
                        cout << intercode_copy[j]->result->name << " ";
                }
                if (intercode_copy[j]->arg1 != NULL)
                {
                    if (intercode_copy[j]->arg1->name[0] == '<')
                        cout << intercode_copy[j]->arg1->intVal << " ";
                    else
                        cout << intercode_copy[j]->arg1->name << " ";
                }
                if (intercode_copy[j]->arg2 != NULL)
                {
                    if (intercode_copy[j]->arg2->name[0] == '<')
                        cout << intercode_copy[j]->arg2->intVal << " ";
                    else
                        cout << intercode_copy[j]->arg2->name << " ";
                }
                if (intercode_copy[j]->fun != NULL)
                    cout << intercode_copy[j]->fun->name << " ";
                cout << endl;
            }
        }
    }
