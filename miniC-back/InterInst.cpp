...
void InterInst::loadVar(string reg32, string reg8, Var *var)
{
    ...
    if (!var->literal)
    {
        if (!offset)
        {
            emit("mov %s,OFFSET FLAT:%s", reg, name);
        }
        else
        {
            if (!var->isArray)
                emit("mov %s,%s PTR [ebp%+d]", reg, type, offset);
            else
    ...
    {
        if (var->isBase())
            emit("mov %s,%d", reg, var->intVal);
        else
            emit("lea %s,[ebp%+d]", reg, offset); //error
    }
}
...
void InterInst::storeVar(string reg32, string reg8, Var *var)
{
    ...
    int off = var->offset;
    if (!off)
        emit("mov OFFSET:FLAT:%s,%s", name, reg);
    else
        emit("mov %s PTR[ebp%+d],%s", type, off, reg);
}
void InterInst::initVar(Var *var)
{
    ...
    {
        if (var->isBase())
            emit("mov eax,%d", var->intVal);
        else if (!var->initdata->isBase())
            emit("mov eax,OFFSET FLAT:%s", var->initdata->name.c_str());
        else if (var->initdata->isBase())
            emit("mov eax,%s PTR [ebp%+d]", type, var->initdata->offset);
        storeVar("eax", "al", var);
    }
}
void InterInst::toX86()
{
    ...
    case OP_ENTRY:
        ...
        if (this->fun->name == "main")
        {
            ...
            emit("sub esp,%d", this->fun->maxDepth);
        if (this->fun->name == "main")
        {
            //emit("call	___main");
        }
    ...
    case OP_PRINTF:
        ...
        {
            emit("mov	DWORD PTR [esp], OFFSET FLAT:%s\n", arg1->name.c_str());
    ...
    case OP_READ:
        if (result->isBase() && !result->isPara)
        {
            ...
            emit("mov	DWORD PTR [esp+4], eax");
            emit("mov	DWORD PTR [esp], OFFSET FLAT:%s", arg1->name.c_str());
            emit("call	scanf");
        }
