...
_TokenType Parser::type()
{
    ...
    if (F(_INT) _(_CHAR) _(_VOID))
    {
        ...
        recovery(F(ID) _(TIMES), TYPE_LOST, TYPE_WRONG);
    depth--;
    return STR;
}
...
Var *Parser::varrdef(bool ext, _TokenType _type, bool ptr, string name)
{
    ...
    if (match(LBRACK))
    {
        ...
        if (!match(RBRACK))
            recovery(F(SEMICON) _(COMMA), RBRACK_LOST, RBRACK_WRONG);
...
Var *Parser::init(bool ext, _TokenType _type, bool ptr, string name)
{
    ...
    else if (ptr == true && initVal != NULL && !initVal->isPtr)
        temp->pointTo = initVal;
    //symtab->addVar(temp);
    //symtab->ShowVarTab();
    depth--;
    return temp;
...
void Parser::deflist(bool ext, _TokenType _type)
{
    ...
    if (match(COMMA))
    {
        ...
        recovery(F(_INT) _(_VOID) _(_CHAR) _(_WHILE) _(_FOR) _(_FOR) _(_DO) _(_IF) _(_SWITCH) _(_BREAK) _(_CONTINUE) _(_RETURN) _(RBRACE), SEMICON_LOST, SEMICON_WRONG);
    depth--;
}
...
void Parser::def(bool ext, _TokenType _type)
{
    ...
    else if (match(TIMES))
    {
        ...
        recovery(F(SEMICON) _(COMMA) _(ASSIGN) _(LBRACK) _(LPAREN), ID_LOST, ID_WRONG);
    depth--;
}
/**
...
void Parser::idtail(bool ext, _TokenType _type, bool ptr, string name)
{
    ...
    if (match(LPAREN))
    {
        ...
        para(paraList);
        if (!match(RPAREN))
            recovery(F(SEMICON) _(LBRACE), RPAREN_LOST, RPAREN_WRONG);
...
Var *Parser::paradata(_TokenType _type)
{
    ...
    else if (match(TIMES))
    {
        if (F(ID))
        {
            ...
            recovery(F(RPAREN) _(COMMA), ID_LOST, ID_WRONG);
    }
    else
        recovery(F(RPAREN) _(COMMA), ID_LOST, ID_WRONG);
    depth--;
...
/**
** <paradatatail> -> [NUM]|ε
* follow(paradatatail) 不需要考虑ε,后续程序会发现错误？
*/
Var *Parser::paradatatail(_TokenType _type, string name)
{
    ...
    if (match(LBRACK))
    {
        ...
        if (F(NUM))
            mov();
        else if (!match(RBRACK))
            recovery(F(RPAREN) _(COMMA), RBRACK_LOST, RBRACK_WRONG);
        depth--;
...
void Parser::block()
{
    ...
    data.push_back(new DrawTreeData(drawCol, depth, (char *)__FUNCTION__));
    if (!match(LBRACE))
        recovery(F(_INT) _(_VOID) _(_CHAR) _(_WHILE) _(_FOR) _(_FOR) _(_DO) _(_IF) _(_SWITCH) _(_BREAK) _(_CONTINUE) _(_RETURN) _(RBRACE), LBRACE_LOST, LBRACE_WRONG);
    subprogram();
    if (!match(RBRACE))
        recovery(F(_INT) _(_VOID) _(_CHAR) _(_WHILE) _(_FOR) _(_FOR) _(_DO) _(_IF) _(_SWITCH) _(_BREAK) _(_CONTINUE) _(_RETURN) _(END), RBRACE_LOST, RBRACE_WRONG);
    depth--;
}

/**
** <subprogram> -><localdef><subprogram>|<statement><subprogram>|kong
*
*/
...
Var *Parser::asstail(Var *&lval)
{
    ...
    if (F(ASSIGN) _(PLUS_ASSIGN) _(MINUS_ASSIGN) _(DIV_ASSIGN) _(TIMES_ASSIGN))
    {
        ...
        Var *result = NULL;
        if (op == ASSIGN)
            result = ir->genTwoOp(lval, ASSIGN, rval);
        else if (op == PLUS_ASSIGN)
            result = ir->genAdd(lval, lval, rval);
        else if (op == MINUS_ASSIGN)
            result = ir->genSub(lval, lval, rval);
        else if (op == TIMES_ASSIGN)
            result = ir->genMul(lval, lval, rval);
        else if (op == DIV_ASSIGN)
            result = ir->genDiv(lval, lval, rval);
        depth--;
        return result;
    }
    depth--;
...
Var *Parser::elem()
{
    ...
    else if (match(LPAREN))
    {
        ...
        if (!match(RPAREN))
            recovery(F(INC) _(DEC), RPAREN_LOST, RPAREN_WRONG);
        depth--;
        return result;
    }
    else
...
Var *Parser::idexpr(string name)
{
    ...
    if (match(LBRACK))
    {
        ...
        Var *index = expr();
        if (!match(RBRACK))
            recovery(F(INC) _(DEC) _(ASSIGN), RBRACK_LOST, RBRACK_WRONG);
    ...
    else if (match(LPAREN))
    {
        ...
        if (!match(RPAREN))
            recovery(F(INC) _(DEC) _(ID) _(LPAREN) _(PLUS) _(MINUS) _(TIMES) _(DIV) _(PLUS_ASSIGN) _(MINUS_ASSIGN) _(TIMES_ASSIGN) _(DIV_ASSIGN) _(LOGICAL_AND) _(LOGICAL_OR) _(ASSIGN) _(SEMICON) _(GE) _(GT) _(LE) _(LT) _(EQU) _(NEQU), RPAREN_LOST, RPAREN_WRONG);
        /**< 解决赋值问�? */
...
Var *Parser::literal()
{
    ...
    if (F(NUM) _(CH) _(STR))
    {
        ...
        recovery(F(ASSIGN) _(LOGICAL_AND) _(LOGICAL_OR) _(GE) _(GT) _(LE) _(EQU) _(NEQU) _(PLUS) _(MINUS) _(TIMES) _(DIV) _(MOD) _(INC) _(DEC) _(COMMA) _(SEMICON), LITERAL_LOST, LITERAL_WRONG);
    depth--;
    return v;
}
/**
...
void Parser::whilestat()
{
    ...
    match(_WHILE);
    if (!match(LPAREN))
        recovery(F(RPAREN) _(ID) _(BIT_NOT) _(MINUS) _(BIT_AND) _(TIMES) _(INC) _(DEC) _(LPAREN) _(NUM) _(_CHAR) _(STR), LPAREN_LOST, LPAREN_WRONG); //ÈçºÎÅÐ¶Ï<altexpr>µÄfirst¼¯
    drawCol.push_back(depth);
    Var *cond = altexpr();
    ir->genWhileCond(cond, _exit);

    if (!match(RPAREN))
        recovery(F(LBRACE), RPAREN_LOST, RPAREN_WRONG); //<block> first is {LBRACE}
    drawCol.pop_back();
    block();
...
void Parser::dowhilestat()
{
    ...
    if (!match(LPAREN))
        recovery(F(ID) _(BIT_NOT) _(MINUS) _(BIT_AND) _(TIMES) _(INC) _(DEC) _(LPAREN) _(NUM) _(_CHAR) _(STR) _(SEMICON), LPAREN_LOST, LPAREN_WRONG); //not sure
...
void Parser::forstat()
{
    ...
    match(_FOR);
    if (!match(LPAREN))
        recovery(F(ID) _(BIT_NOT) _(MINUS) _(BIT_AND) _(TIMES) _(INC) _(DEC) _(LPAREN) _(NUM) _(_CHAR) _(STR) _(SEMICON) _(_INT) _(_VOID) _(_CHAR), LPAREN_LOST, LPAREN_WRONG);
    drawCol.push_back(depth);
    forinit();
    ...
    if (!match(SEMICON))
        recovery(F(ID) _(BIT_NOT) _(MINUS) _(BIT_AND) _(TIMES) _(INC) _(DEC) _(LPAREN) _(NUM) _(_CHAR) _(STR) _(SEMICON), SEMICON_LOST, SEMICON_WRONG);
    altexpr();
    if (!match(RPAREN))
...
void Parser::forinit()
{
    depth++;
    data.push_back(new DrawTreeData(drawCol, depth, (char *)__FUNCTION__));
    if (F(_INT) _(_VOID) _(_CHAR))
        localdef(false, look->type);
    else
    {
        altexpr();
        if (!match(SEMICON))
            recovery(F(ID), SEMICON_LOST, SEMICON_WRONG);
    }
    depth--;
}
...
void Parser::ifstat()
{
    ...
    match(_IF);
    if (!match(LPAREN))
        recovery(F(ID) _(BIT_NOT) _(MINUS) _(BIT_AND) _(TIMES) _(INC) _(DEC) _(LPAREN) _(NUM) _(_CHAR) _(STR), LPAREN_LOST, LPAREN_WRONG);
    drawCol.push_back(depth);
    Var *cond = expr();
...
void Parser::switchstat()
{
    ...
    if (!match(RPAREN))
        recovery(F(LBRACE), RPAREN_LOST, RPAREN_WRONG);
    if (!match(LBRACE))
        recovery(F(_CASE) _(_DEFAULT), LBRACE_LOST, LBRACE_WRONG);
    drawCol.pop_back();
    casestat(cond);

    if (!match(RBRACE))
        recovery(F(_INT) _(_VOID) _(_CHAR) _(_WHILE) _(_FOR) _(_FOR) _(_DO) _(_IF) _(_SWITCH) _(_BREAK) _(_CONTINUE) _(_RETURN) _(RBRACE), RBRACE_LOST, RBRACE_WRONG);
...
void Parser::casestat(Var *cond)
{
    ...
    if (match(_CASE))
    {
        ...
        if (!match(COLON))
            recovery(F(_INT) _(_VOID) _(_CHAR) _(_WHILE) _(_FOR) _(_FOR) _(_DO) _(_IF) _(_SWITCH) _(_BREAK) _(_CONTINUE) _(_RETURN) _(RBRACE), COLON_LOST, COLON_WRONG);
        symtab->enter();
        subprogram();
        symtab->leave();
...
void Parser::DrawTree(bool outOpt)
{
    ...
    for (int i = 0; i < data.size(); i++)
    {
        ...
        {
            cout << "---->\"" << data[i]->funName << "\" ";
            if (i < data.size() - 1 && data[i + 1]->depth != -1)
                cout << endl;
            continue;
        }
        vector<int> &temp = data[i]->drawCol;
        //cout<<data[i]->depth<<endl;
        int number = temp.back(); /**< 如果size()==1 则number�?0 */
        if (temp.size() == 1)
        {
        ...
        {
            for (int j = 1; j < temp.size(); j++) /**< 数组元素中间的空格以及竖�? */
            {
                //cout<<temp[j-1]<<endl;
                int a = temp[j - 1]; /**< 画空�? */
                while (a < temp[j])
...
void Parser::readstat() /**< 输入单个变量 */
{
    ...
    if (F(ID))
    {
        ...
        if (match(LBRACK))
        {
            ...
            Var *index = expr();
            if (!match(RBRACK))
                recovery(F(RPAREN), RBRACK_LOST, RBRACK_WRONG);
            result = ir->genArray(val, index);
        }
        else
            result = symtab->getVar(name);
        Var *val = symtab->getVar(name);
        if(val!=NULL){
            if (val->type == _INT)
            {
    ...
    {
        if (!F(ID))
            recovery(F(RPAREN) _(SEMICON), ID_LOST, ID_WRONG);
        else
        {
            name = ((Id *)look)->name;
            Var *val = symtab->getVar(name);
